...
if(isset($_GET['key'])){
           ...
           $resultsperpage=10;
           $stmt=$con->prepare("SELECT * FROM project_underconstruction_details
...
$data=$stmt->fetchAll();
           ...
           $start=($page-1)*$resultsperpage;
            ...
            $stmt=$con->prepare("SELECT * FROM project_underconstruction_details ORDER BY project DESC LIMIT $start,$resultsperpage");
            $stmt->execute();
...
foreach($data as $row){
  $underconstruction_project_pic=$row['underconstruction_project_pic'];
  $img=ltrim($underconstruction_project_pic,".");
  $project=$row['project'];
...
<div class="row">
   ...
   <?php
                if($page<$totalpages){
                     echo "<a href='underconstruction.php?page=".($page+1)."'>Next&gt;&gt;</a>";
                     echo "&nbsp;";
                     echo "<a href='underconstruction.php?page=".$rowcount."'>Last&gt;&gt;</a>";  
                     }

                if($page>=2){   
                     echo "<a href='underconstruction.php?page=".($page-1)."'>  &lt;&lt;Prev </a>";
                     echo "&nbsp;";
                     echo "<a href='underconstruction.php?page=1'>&lt;&lt;First</a>"; 
                     echo "&nbsp;";   
                     }
